...
1. **No manual override.** 03 §2 (sitemap) and the General page spec call for **Appearance:
   System / Light / Dark**. A user who keeps macOS light but wants Velata dark has no path.
...
> hint: _"Match macOS, or force light or dark for Velata's windows."_
...
| Dictionary empty  | `"No entries yet."` `DictionaryTab.tsx:70`                                                                 | dead end                                              | worked example ghost row (05 §3.1): _"Nothing here yet. When a name or term gets misheard, add it — e.g. 'open flow' → 'Velata'."_ | UX-10        |
...
| Onboarding        | Accessibility copy frames as paste-only `Onboarding.tsx:96-100`                                            | hides Rewrite needs it too                            | add _"…it also lets Velata read selected text for Rewrite. Skip and you'll get clipboard-only paste, and Rewrite won't work."_     | UX-26        |
...
callout when `!accessibility`: _"Accessibility is off — Velata copies to the clipboard
