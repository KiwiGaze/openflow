...
`check:ipc` (scripts/check-ipc.mjs) fails on Rust↔TS IPC drift; `check:privacy` (scripts/check-privacy.mjs) fails on network code outside `llm.rs`/`models.rs`/`cloud_stt.rs`, webview network I/O, or telemetry deps. Build first matters: `@openflow/desktop` imports `@openflow/core` from its `dist/`.
