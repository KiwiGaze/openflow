...
- **Persistence.** One settings JSON file (`~/Library/Application Support/app.velata.desktop/settings.json`), camelCase, schema-versioned, atomically written. Models are ggml files under `<app-data>/models/`; LLM profiles are one JSON file each under `<app-data>/profiles/` and cloud STT profiles under `<app-data>/stt-profiles/` (same pattern, both: filename stem = identity, 0600, corrupt files skipped never deleted; `activeLlmProfileId` in settings points at the active LLM profile, "" = no AI). Durable structured data lives in one SQLite file (`<app-data>/velata.db`, rusqlite bundled, WAL — `db.rs`): tables `history`, `notes`, `note_versions` (all opt-in user content; soft-deleted / capped, never destroyed) and `insights_daily` (counts and dates only, opt-in). Invariants: the app-data dir is 0700 and the db file 0600; schema migrates forward through `PRAGMA user_version`, each step inside one transaction; a corrupt/unopenable db is renamed aside to `velata.db.corrupt-<ts>` (never deleted) and a fresh one created. Audio is **never** persisted — that invariant is absolute. All three SQLite stores are off by default — history and transcripts behind `historyEnabled` (text only, capped, retention `historyRetentionDays`, clearable → `history.rs`), all-time `insights_daily` behind `appStatsEnabled` (counts/dates, never words or audio), Scratchpad notes behind `scratchpadEnabled` (`notes.rs`) — so with the defaults nothing is stored. Nothing else is written.
